...
class Month implements Parcelable {
  ...
  public static final Creator<Month> CREATOR = new Creator<Month>() {
    public com.example.lunarcalendar.lunarview.Month createFromParcel(Parcel in) {
      return new com.example.lunarcalendar.lunarview.Month(in);
    }

    public com.example.lunarcalendar.lunarview.Month[] newArray(int size) {
      return new com.example.lunarcalendar.lunarview.Month[size];
    }
  };
  ...
  private Calendar generateWorkingCalendar(int year, int month, int day) {
    ...
    totalWeeks = (totalDays + delta) / DAYS_IN_WEEK + ((totalDays + delta) % DAYS_IN_WEEK != 0 ? 1 : 0);
  ...
  private boolean isMonthOfToday(Calendar calendar) {
    ...
    return calendar.get(Calendar.YEAR) == today.get(Calendar.YEAR) && calendar.get(Calendar.MONTH) == today.get(Calendar.MONTH);
  }
